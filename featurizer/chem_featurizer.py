...
from featurizer.featurizer import (
    OneHotFeaturizer,
    Featurizer,
    FeaturizerList,
    LambdaFeaturizer,
...
atom_atomic_number = LambdaFeaturizer(
    lambda atom: [atom.GetAtomicNum()], length=1, name="atomic_number"
)
...
atom_total_degree = LambdaFeaturizer(
    lambda atom: [atom.GetTotalDegree()], length=1, name="atom_total_degree"
)
...
atom_degree = LambdaFeaturizer(
    lambda atom: [atom.GetDegree()], length=1, name="atom_degree"
)
...
atom_implicit_valence = LambdaFeaturizer(
    lambda atom: [atom.GetImplicitValence()], length=1, name="atom_implicit_valence"
)
...
atom_explicit_valence = LambdaFeaturizer(
    lambda atom: [atom.GetExplicitValence()], length=1, name="atom_explicit_valence"
)
...
atom_formal_charge = LambdaFeaturizer(
    lambda atom: [atom.GetFormalCharge()], length=1, name="atom_formal_charge"
)

atom_mass = LambdaFeaturizer(
    lambda atom: [atom.GetMass() * 0.01], length=1, name="atom_mass"
)
...
atom_total_num_H = LambdaFeaturizer(
    lambda atom: [atom.GetTotalNumHs()], length=1, name="atom_total_num_H"
)

atom_is_aromatic = LambdaFeaturizer(
    lambda atom: [atom.GetIsAromatic()],
    length=1,
    name="atom_is_aromatic",
)

atom_is_in_ring = LambdaFeaturizer(
    lambda atom: [atom.IsInRing()], length=1, name="atom_is_in_ring"
)
