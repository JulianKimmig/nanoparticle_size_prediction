...
class PlotValidationCallback(PytorchCallback):
    def __init__(
        ...
        path=None,
        max_points=None,
        title="correlation true/predicted for {model} in epoch {epoch}",
        animate=True,
        normalize=False,
        *args,
        **kwargs,
    ...
    def execute(self, epoch, log):
        ...
        if self.normalize:
            ...
            prange = (
                y_true_min - 0.05 * (y_true_max - y_true_min),
                y_true_max + 0.05 * (y_true_max - y_true_min),
            )
        ...
        def customJoint(x, y):
            ...
            if len(y_pred_plot.shape) > 1:
                for i in range(y_true_plot.shape[1]):
                    ...
                    pltable = (y_pred_plot[:, i] >= prange[0]) & (
                        y_pred_plot[:, i] <= prange[1]
                    )
                    plt.scatter(
                        y_true_plot[:, i][pltable],
                        y_pred_plot[:, i][pltable],
                        alpha=0.6,
                        label=label,
                    )
        ...
        def customMarginal(x, vertical=True, *args, **kwargs):
            ...
            if len(ar.shape) > 1:
                for i in range(ar.shape[1]):
                    ...
                    hist1 = barf(
                        h[1][:-1] + (prange[1] - prange[0]) / (2 * bins),
                        h[0] / h[0].max(),
                        (prange[1] - prange[0]) / bins,
                    )
        ...
        i = 0
        max_length = 30
        title = self.title.replace("{epoch}", str(epoch)).replace(
        ...
        if self.animate:
            if len(self._images) == 0:
                ...
                self._images.extend(
                    [
                        imageio.imread(os.path.join(self.dir, f"{i}.png"))
                        for i in sorted(available_epoch_images)
                    ]
                )
            self._images.append(imageio.imread(os.path.join(self.dir, f"{epoch}.png")))
            imageio.mimsave(
                os.path.join(self.dir, f"{self.name}.gif"), self._images, fps=5
            )
