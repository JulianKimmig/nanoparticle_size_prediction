...
class PytorchModel():
    ...
    def save(self, filename=None):
        ...
        torch.save({'model_state_dict': self.module.state_dict(),
                    "model_log": self.log,
                    }, filename)
    ...
    def train(self, data_loader, epochs=1, validation_loader=None,
        ...
        if verbose:
            ...
            def _print_batch_step(type, batch, batch_len, len_labels, batch_time_start, step_time_start, meter):
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for batch_id, batch_data in enumerate(data_loader):
               ...
               if self.model.batch_data_converter:
                ...
                if verbose:
                    _print_batch_step(type="train", batch=batch_id, batch_len=batch_len, len_labels=len(y),
                                      batch_time_start=batch_time_start, step_time_start=step_time_start,
                                      meter=train_meter)

            if verbose:
                print()
            train_score = {m: np.mean(train_meter.compute_metric(m)) for m in self.metrics}
            ...
            if validation_loader:
                ...
                with torch.no_grad():
                    for batch_id, batch_data in enumerate(validation_loader):
                        ...
                        if verbose:
                            _print_batch_step(type="valid", batch=batch_id, batch_len=batch_len, len_labels=len(y),
                                              batch_time_start=batch_time_start, step_time_start=step_time_start,
                                              meter=eval_meter)

                if verbose:
                    print()
                valid_score = {m: np.mean(eval_meter.compute_metric(m)) for m in self.metrics}
            ...
            if verbose:
                ...
                lines = []
                lines.append(f"training time: {((ct - training_start) * ureg.s).to_compact():.3f~}")
                ...
                if len(callback_times) > 0:
                    ...
                    for cbt in callback_times:
                        lines.append(f"  {cbt[0]} {(cbt[1] * ureg.s).to_compact():.3f~}")
        ...
        if test_loader is not None:
            ...
            with torch.no_grad():
                ...
                for batch_id, batch_data in enumerate(test_loader):
                    ...
                    if verbose:
                        _print_batch_step(type="test", batch=batch_id, batch_len=batch_len, len_labels=len(y),
                                          batch_time_start=batch_time_start, step_time_start=step_time_start,
                                          meter=eval_meter)

            test_score = {m: np.mean(eval_meter.compute_metric(m)) for m in self.metrics}
            if verbose:
                print()
