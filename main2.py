...
def load_model(config, load=True):
    ...
    if backend == "pytorch_geometric":
        ...
        import graph_backends.dgl_based as backend
    else:
    ...
    def model_regress(smodel, data, device):
        ...
        print(smiles, bg, addi)

        h = torch.from_numpy(bg.features).float()
        h = h.to(device)
        addi = addi.to(device)
...
if __name__ == '__main__':
    ...
    def_conf= os.path.join(os.path.expanduser("~"), ".smartchem","np_model_{}".format(int(time.time())),"config.json")

    parser.add_argument('--config', type=str, default=None,
                        help='path to the config file, default "{}"'.format(
                            os.path.join(os.path.expanduser("~"), ".smartchem","np_model_{int(time.time())}","config.json")
                        ))
...
# from featurizer import load_featurizer
# from featurizer.chem_featurizer import default_atom_featurizer
#
# from model.gcn import GCNMultiInputPredictor2
# import numpy as np

# from model.layer import Flatten
# from model.layer import Flatten
# import model.loss as model_loss
# from model.layer.graph_convolution.graph import mol_to_graph, FixedGraph, merge_graphs
# from model.torch_model import PytorchModel
