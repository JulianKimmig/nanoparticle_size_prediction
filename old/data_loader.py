...
class MoleculeAdditionalInputCSVDataset(object):
    """MoleculeCSVDataset
    ...
    def __init__(self, df, smiles_to_graph, node_featurizer, edge_featurizer,
                 smiles_column, cache_file_path, task_names=None, additional_input_names=None, load=True,
                 log_every=1000):
        if additional_input_names is None:
            additional_input_names = []

        self.df = df
        self.smiles = self.df[smiles_column].tolist()
        self.additional_input_names = additional_input_names
        if task_names is None:
            self.task_names = self.df.columns.drop([smiles_column] + additional_input_names).tolist()
        else:
    ...
    def _pre_process(self, smiles_to_graph, node_featurizer, edge_featurizer, load, log_every):
        ...
        * Set missing labels to be 0 and use a binary masking
        ...
        if os.path.exists(self.cache_file_path) and load:
            ...
            self.additional_inputs = label_dict['additional_inputs']
        else:
            ...
            _add_input_values = self.df[self.additional_input_names].values
            # np.nan_to_num will also turn inf into a very large number
            self.additional_inputs = F.zerocopy_from_numpy(np.nan_to_num(_add_input_values).astype(np.float32))
            ...
            save_graphs(self.cache_file_path, self.graphs,
                        labels={'labels': self.labels, 'mask': self.mask, 'additional_inputs': self.additional_inputs})
    ...
    def __getitem__(self, item):
        ...
        """
        return self.smiles[item], self.graphs[item], self.labels[item], self.additional_inputs[item]
    ...
    @staticmethod
    def collate(data):
        smiles, graphs, labels, additional_inputs = map(list, zip(*data))
        bg = dgl.batch(graphs)
        bg.set_n_initializer(dgl.init.zero_initializer)
        bg.set_e_initializer(dgl.init.zero_initializer)
        labels = torch.stack(labels, dim=0)
        additional_inputs = torch.stack(additional_inputs, dim=0)

        return (smiles, bg, additional_inputs), labels

    @staticmethod
    def model_regress(smodel, data, device):
        smiles, bg, addi = data

        h = bg.ndata.pop('nfeats')
        h = h.to(device)
        addi = addi.to(device)
