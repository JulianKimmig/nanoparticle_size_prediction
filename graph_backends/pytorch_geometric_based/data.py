...
def dataframe_to_data_predict_set(df, to_graph, graph_columns, batch_size=32, add_kwargs={}):
    ...
    for i, row in df.iterrows():
        ...
        for key, col_list in add_kwargs.items():
            _add_kwargs[key] = torch.from_numpy(np.expand_dims(row[col_list].values.astype(float), 0)).float()
...
def dataframe_to_data_train_sets(df, to_graph, graph_columns, task_cols, batch_size=32, add_kwargs={}, seed=None,
    ...
    for i, row in df.iterrows():
        ...
        if rep_gdi in gd:
            ...
            graph_data = to_graph(gdi)
            gd[rep_gdi] = graph_data
        y = torch.from_numpy(np.expand_dims(row[task_cols].values.astype(float), 0)).float()
    ...
    for i in split[:-1]:
        ...
        sets.append(DataLoader(data_list[start:end], batch_size=batch_size, shuffle=shuffle))
        start = end
    end = original_length
    sets.append(DataLoader(data_list[start:end], batch_size=batch_size, shuffle=shuffle))
